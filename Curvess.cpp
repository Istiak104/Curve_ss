...
double curve[200],y,width=1280,height=720,mid=360,radius=1,balx,dx,dx2,Y=0,dx1,baly[200],paused=0,tracer[200],dobx,cur_y,deg,doby,instruction=280;
int j=3;
...
void addgraph()
{
    if(!hide){
        ...
        if(amplitude[j]>0) amplitude[j]=min(amin,amplitude[j]);
        else if (amplitude[j]<0) amplitude[j]=max(-amin,amplitude[j]);
        amp[j]=amplitude[j];
...
void iKeyboard(unsigned char key)
{
    ...
    if(key=='A')
    {
        for(i=0;i<j;i++)
        {
            ...
            {
                amplitude[i]-=1;
                amplitude[i]=max(amplitude[i],-amin);
            ...
            {
                amplitude[i]+=1;
                amplitude[i]=min(amplitude[i],amin);
                if(!hide) amp[i]=amplitude[i];
                amplitude1[i]=amplitude[i];
            }
    ...
    if(key=='a')
    {
        for(i=0;i<=j;i++)
        {
                ...
                {
                    amplitude[i]-=1;
                    amplitude[i]=max(amax,amplitude[i]);
                    if(!hide) amp[i]=amplitude[i];
                    amplitude1[i]=amplitude[i];
                }
        }
